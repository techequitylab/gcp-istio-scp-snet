...
mkdir -p `pwd`/gcp-istio-scp-snet > /dev/null 2>&1
export SCRIPTNAME=gcp-istio-scp-snet.sh
export PROJDIR=`pwd`/gcp-istio-scp-snet
...
export GCP_CLUSTER_1=hipster-scp-snet-1
...
export GCP_CLUSTER_2=hipster-scp-snet-2
...
cat<<EOF
=======================================================
Menu for Istio Single Control Plane, Single Network 
-------------------------------------------------------
...
source $PROJDIR/.env
    ...
    do
        if [ $MODE -eq 1 ]; then
            ...
            if [ "$i" -eq 1 ]; then
                echo
                echo "$ gcloud --project $PROJECT beta container clusters create $CLUSTER --zone $ZONE --machine-type e2-standard-2 --num-nodes 2 --network $APPLICATION_NAME-net --subnetwork $APPLICATION_NAME-subnet-${i} --enable-ip-alias --spot regular # to create cluster" | pv -qL 100
                gcloud beta container clusters create $CLUSTER --zone $ZONE --machine-type e2-standard-2 --num-nodes 2 --network $APPLICATION_NAME-net --subnetwork $APPLICATION_NAME-subnet-${i} --enable-ip-alias --spot
            else
                echo
                echo "$ gcloud --project $PROJECT beta container clusters create $CLUSTER --zone $ZONE --machine-type e2-standard-2 --num-nodes 2 --network $APPLICATION_NAME-net --subnetwork $APPLICATION_NAME-subnet-${i} --spot # to create cluster" | pv -qL 100
                gcloud --project $PROJECT beta container clusters create $CLUSTER --zone $ZONE --machine-type e2-standard-2 --num-nodes 2 --network $APPLICATION_NAME-net --subnetwork $APPLICATION_NAME-subnet-${i} --spot
            fi
            echo
            echo "$ kubectl config use-context $CTX # to set context" | pv -qL 100
...
spec:
  ...
  values:
    istiodRemote:
      injectionPath: /inject/cluster/$\REMOTE_CLUSTER/net/\$APPLICATION_NAME-net-\${i}
    global:
...
EOF
            ...
            echo
            echo "$ $HOME/istio-${ISTIO_VERSION}/samples/multicluster/gen-eastwest-gateway.sh --mesh mesh1 --cluster $HOST_CLUSTER --network $APPLICATION_NAME-net | $HOME/istio-${ISTIO_VERSION}/bin/istioctl --context $HOST_CTX install -y -f - # to install a gateway dedicated to east-west traffic" | pv -qL 100
            $HOME/istio-${ISTIO_VERSION}/samples/multicluster/gen-eastwest-gateway.sh --mesh mesh1 --cluster $HOST_CLUSTER --network $APPLICATION_NAME-net | $HOME/istio-${ISTIO_VERSION}/bin/istioctl --context $HOST_CTX install -y -f -
            echo && echo
            echo "$ kubectl -n istio-system patch service istio-eastwestgateway -p '{\"metadata\": {\"annotations\": {\"cloud.google.com/load-balancer-type\": \"Internal\"}}}' # to create internal load balancer" | pv -qL 100
...
EOF
            ...
            echo
            echo "$ $HOME/istio-${ISTIO_VERSION}/samples/multicluster/gen-eastwest-gateway.sh --mesh mesh1 --cluster ${REMOTE_CLUSTER} --network $APPLICATION_NAME-net | $HOME/istio-${ISTIO_VERSION}/bin/istioctl --context $REMOTE_CTX install -y -f - # to install a gateway dedicated to east-west traffic" | pv -qL 100
            $HOME/istio-${ISTIO_VERSION}/samples/multicluster/gen-eastwest-gateway.sh --mesh mesh1 --cluster ${REMOTE_CLUSTER} --network $APPLICATION_NAME-net | $HOME/istio-${ISTIO_VERSION}/bin/istioctl --context $REMOTE_CTX install -y -f -
            echo
            echo "$ kubectl -n istio-system patch service istio-eastwestgateway -p '{\"metadata\": {\"annotations\": {\"cloud.google.com/load-balancer-type\": \"Internal\"}}}' # to create internal load balancer" | pv -qL 100
            kubectl -n istio-system patch service istio-eastwestgateway -p '{"metadata": {"annotations": {"cloud.google.com/load-balancer-type": "Internal"}}}'
            echo
